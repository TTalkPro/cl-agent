...
;;;;
;;;; 说明：
;;;;   这些测试不发真实网络请求——把 cl-agent.http::http-request 临时
...
*ha-request-id* 的可见值。"
  (let ((original (fdefinition 'cl-agent.http::http-request)))
    (unwind-protect
         (progn
           (setf (fdefinition 'cl-agent.http::http-request)
           ...
           (funcall fn))
      (setf (fdefinition 'cl-agent.http::http-request) original))))
...
(defmacro with-http-pool ((&key (size 2)) &body body)
  ...
  `(progn
     (cl-agent.http:initialize-http-thread-pool :size ,size)
     (unwind-protect (progn ,@body)
       (cl-agent.http:shutdown-http-thread-pool))))
      ...
      (let ((f nil))
        (cl-agent.core:with-inherited-specials (*ha-request-id*)
          (let ((*ha-request-id* "req-42"))
            (setf f (cl-agent.http:http-get-async "http://probe.invalid/x"))))
        ;; 此处 let 已退出，*ha-request-id* 已恢复为 NIL
        (is (null *ha-request-id*))
        ;; 但请求体看到的仍是提交时刻的值
        (is (equal "req-42"
                   (getf (cl-agent.http:http-future-value f) :req-id)))))))
  ...
  (with-http-probe
    (with-http-pool ()
      (let ((f (let ((cl-agent.core:*inherited-special-variables* '(*ha-request-id*))
                     (*ha-request-id* "req-7"))
                 (cl-agent.http:http-get-async "http://probe.invalid/y"))))
        (is (equal "req-7" (getf (cl-agent.http:http-future-value f) :req-id)))))))
...
(test http-async-no-inherit-list-still-works
  ...
  (with-http-probe
    (with-http-pool ()
      (let ((f (cl-agent.http:http-get-async "http://probe.invalid/z")))
        (is (equal "http://probe.invalid/z"
                   (getf (cl-agent.http:http-future-value f) :url)))))))
...
(test http-pool-shutdown-reclaims-threads
  ...
  (let ((before (ha-pool-thread-count)))
    (cl-agent.http:initialize-http-thread-pool :size 3)
    ...
    (is (null cl-agent.http::*http-thread-pool*))))

(test http-pool-shutdown-idempotent
  "重复 shutdown 不报错"
  (cl-agent.http:initialize-http-thread-pool :size 2)
  (cl-agent.http:shutdown-http-thread-pool)
  (finishes (cl-agent.http:shutdown-http-thread-pool))
  (finishes (cl-agent.http:shutdown-http-thread-pool)))
