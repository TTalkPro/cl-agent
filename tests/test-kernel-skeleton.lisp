...
(test build-kernel-basic
  ...
  (let* ((dummy-model (list :fake-model))
         ...
         (k (cl-agent/core:build-kernel
             ...
             :filters (list f1))))
    (is (eq dummy-model (cl-agent/core:kernel-model k)))
    (is (equal '(get-weather save-note) (cl-agent/core:kernel-tools k)))
    (is (= 1 (length (cl-agent/core:kernel-filters k))))
    (is (eq :f1 (cl-agent/core:filter-name (first (cl-agent/core:kernel-filters k)))))))

(test build-kernel-defaults
  "build-kernel 缺省值：filters 为空列表，eligibility-fn 为 constantly t"
  (let ((k (cl-agent/core:build-kernel :model (list :fake))))
    (is (null (cl-agent/core:kernel-tools k)))
    (is (null (cl-agent/core:kernel-filters k)))
    (is (null (cl-agent/core:kernel-settings k)))
    (is (funcall (cl-agent/core:kernel-eligibility-fn k) nil nil)
...
(test build-kernel-custom-eligibility
  ...
  (let ((k (cl-agent/core:build-kernel
            ...
            :eligibility-fn (lambda (resp ctx)
                               (declare (ignore resp))
                               (getf ctx :has-budget)))))
    (is (funcall (cl-agent/core:kernel-eligibility-fn k) nil '(:has-budget t)))
    (is (not (funcall (cl-agent/core:kernel-eligibility-fn k) nil '(:has-budget nil))))))
...
(test build-kernel-settings-alist
  ...
  (let ((k (cl-agent/core:build-kernel
    ...
    (is (equal '((:max-tool-iterations . 10) (:timeout . 30))
               (cl-agent/core:kernel-settings k)))))
...
(test kernel-has-no-memory-slot
  ...
  (let ((slot-names (mapcar (lambda (s) (symbol-name (closer-mop:slot-definition-name s)))
                            (closer-mop:class-slots (find-class 'cl-agent/core:kernel)))))
