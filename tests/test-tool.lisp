...
(cl-agent/core:deftool test-adder (&key a b)
  "把两个数相加"
  (:param a :number "第一个数" :required t)
  (:param b :number "第二个数" :required t)
  (+ a b))

(cl-agent/core:deftool test-direct-tool (&key text)
  "直接返回结果的工具"
  (:param text :string "文本")
  (:return-direct t)
  (format nil "直接结果：~A" text))

(cl-agent/core:deftool test-context-tool (&key city tool-context)
  "使用宿主注入上下文的工具"
...
defn + var 元数据，工具由 (build-chat-client {:tools [#'foo]}) 显式传入。"
  (let ((callback (cl-agent/core:symbol-tool-callback 'test-adder)))
    ...
    ;; 未显式注册 → 全局表里查不到
    (is (null (cl-agent/core:find-tool-callback "test_adder")))))

(test register-tool-callback-is-opt-in
  "显式 register-tool-callback 后，字符串名才可解析（配置驱动场景）"
  (let ((cb (cl-agent/core:symbol-tool-callback 'test-adder)))
    (is (null (cl-agent/core:find-tool-callback "test_adder")))
    (unwind-protect
         (progn
           (cl-agent/core:register-tool-callback cb)
           (is (eq cb (cl-agent/core:find-tool-callback "test_adder")))
           (is (equal (list cb) (cl-agent/core:resolve-tool-callbacks
                                 '("test_adder")))))
      (cl-agent/core:unregister-tool-callback "test_adder"))
    (is (null (cl-agent/core:find-tool-callback "test_adder")))))

(test deftool-description-and-params
  "docstring 与 :param 子句进入 tool-definition"
  (let* ((callback (cl-agent/core:symbol-tool-callback 'test-adder))
         (def (cl-agent/core:tool-callback-definition callback)))
    (is (string= "把两个数相加" (cl-agent/core:tool-definition-description def)))
    (is (= 2 (length (cl-agent/core:tool-definition-parameters def))))))

(test deftool-schema
  "自动派生 provider schema"
  (let ((schema (cl-agent/core:tool-callback->schema
                 (cl-agent/core:symbol-tool-callback 'test-adder))))
...
(test deftool-rejects-positional-args
  ...
  (signals error
    (macroexpand-1 '(cl-agent/core:deftool bad-tool (x y)
                     "位置参数不允许"
                     (+ x y)))))
...
(test make-tool-callback-runtime
  ...
  (let ((callback (cl-agent/core:make-tool-callback
                   (lambda (&key name) (format nil "你好，~A" name))
                   :name "greet"
                   :description "打招呼"
                   :parameters '((name :string "姓名" :required-p t)))))
    (is (string= "你好，大卫"
                 (cl-agent/core:tool-callback-call callback '(:name "大卫"))))))

(test tool-callback-non-string-result
  "非字符串结果自动 format"
  (let ((callback (cl-agent/core:make-tool-callback
                   (lambda (&key) 42) :name "answer")))
    (is (string= "42" (cl-agent/core:tool-callback-call callback nil)))))

(test tool-callback-error-wrapped
  "工具内部错误包装为 tool-execution-error"
  (let ((callback (cl-agent/core:make-tool-callback
                   (lambda (&key) (error "内部炸了")) :name "bomb")))
    (signals cl-agent/core:tool-execution-error
      (cl-agent/core:tool-callback-call callback nil))))

(test resolve-tool-callbacks
  "解析工具引用：实例 / 符号（deftool 的默认路径）"
  (let* ((inline-cb (cl-agent/core:make-tool-callback
                     (lambda (&key) "x") :name "inline"))
         (resolved (cl-agent/core:resolve-tool-callbacks
                    (list inline-cb 'test-adder))))
    (is (= 2 (length resolved)))
    (is (eq inline-cb (first resolved)))
    (is (string= "test_adder" (cl-agent/core:tool-callback-name (second resolved)))))
  (signals cl-agent/core:tool-not-found-error
    (cl-agent/core:resolve-tool-callbacks '("no_such_tool")))
  ;; 字符串引用需先显式注册——deftool 不再自动进全局表
  (signals cl-agent/core:tool-not-found-error
    (cl-agent/core:resolve-tool-callbacks '("test_adder"))))
...
(test arguments-normalization
  ...
  (let ((ht (make-hash-table :test #'equal)))
    ...
    (is (equal '(:user-name "david") (cl-agent/core:arguments->plist ht))))
  (is (equal '(:city "东京")
             (cl-agent/core:arguments->plist "{\"city\": \"东京\"}")))
  (is (equal '(:a 1) (cl-agent/core:arguments->plist '(:a 1))))
  (is (null (cl-agent/core:arguments->plist nil)))
  (is (null (cl-agent/core:arguments->plist "不是 JSON"))))
...
(defun opts-exposing (&rest tool-syms)
  ...
  (cl-agent/core:make-chat-options
   :tool-callbacks (cl-agent/core:resolve-tool-callbacks tool-syms)))

(test find-callback-for-call-resolves-exposed
  "本次请求暴露了的工具，按名解析得到"
  (let* ((options (opts-exposing 'test-adder))
         (tc (cl-agent/core:make-tool-call :id "1" :name "test_adder"
                                           :arguments '(:a 1 :b 2)))
         (cb (cl-agent/core:find-callback-for-call options tc)))
    (is (string= "test_adder" (cl-agent/core:tool-callback-name cb)))))

(test find-callback-for-call-rejects-unexposed
  "未暴露的工具名 → tool-not-found-error（不回退全局注册表）"
  (let ((options (opts-exposing 'test-adder))
        (tc (cl-agent/core:make-tool-call :id "1" :name "test_context_tool"
                                          :arguments '(:city "x"))))
    (signals cl-agent/core:tool-not-found-error
      (cl-agent/core:find-callback-for-call options tc))))
...
(cl-agent/core:deftool secret-tool (&key target)
  "危险操作——本测试中从不暴露给模型"
         ...
         (progn
           ...
           (let* ((provider (make-seq-provider
             ...
             (is (null *secret-tool-fired*)
                 "未暴露的工具被执行了：越权回归")))
      (cl-agent/core:unregister-tool-callback "secret_tool")
      (setf *secret-tool-fired* nil))))
...
(cl-agent/core:deftool pkg-weather (&key city)
  "另一个包里定义的工具"
  (:param city :string "城市" :required t)
  (format nil "~A：晴" city))
(in-package :cl-agent/tests)
...
这与 clj-agent 同构（跨 ns 需 #'other-ns/foo 或 :require :refer）。"
  ...
  (is (string= "pkg_weather"
               (cl-agent/core:tool-callback-name
                (first (cl-agent/core:resolve-tool-callbacks
                        '(cl-agent/tests/tool-pkg:pkg-weather))))))
  ;; 本包同名符号是另一个对象 → 解析失败
  (is (not (eq 'pkg-weather 'cl-agent/tests/tool-pkg:pkg-weather)))
  (signals cl-agent/core:tool-not-found-error
    (cl-agent/core:resolve-tool-callbacks '(pkg-weather))))
...
两者都会报出误导性的「找不到工具：PKG-WEATHER」。"
  (let ((message (handler-case
                     (progn (cl-agent/core:resolve-tool-callbacks '(pkg-weather)) nil)
                   (cl-agent/core:tool-not-found-error (e) (princ-to-string e)))))
